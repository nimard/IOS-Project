...
class ViewController6: UIViewController {
    ...
    private func loadCustomView(){
        ...
        if yellowScore > blueScore && yellowScore > greenScore && yellowScore > orangeScore{
            winColor = UIColor.yellow
        }
        if blueScore > yellowScore && blueScore > greenScore && blueScore > orangeScore{
            winColor = UIColor.blue
        }
        if greenScore > yellowScore && blueScore < greenScore && greenScore > orangeScore{
            winColor = UIColor.green
        }
        if orangeScore > yellowScore && orangeScore > greenScore && blueScore < orangeScore{
            winColor = UIColor.orange
        }
    ...
    @IBAction func changeWord(_ sender: Any) {
        ...
        if start == 0 {
            ...
            if check == 0 {
                ...
                if pas[3] == 0 {
                    orangeScore += 1
                    orangeScore -= helpPoint
                    helpPoint = 0
                    orangeScoreLabel.text = String(orangeScore)
                }
                if pas[0] == 0 {
                    timer.invalidate()
                    timer = Timer.scheduledTimer(timeInterval: 0.3, target: self, selector: #selector(blueImageFunc), userInfo: nil, repeats: true)
                ...
                if pas[3] == 0 {
                    timer.invalidate()
                    timer = Timer.scheduledTimer(timeInterval: 0.3, target: self, selector: #selector(orangeImageFunc), userInfo: nil,          repeats: true)
                }else {
                    self.changeWord(1)
                }
            }
    ...
    @objc func orangeImageFunc() {
        let frame = CGRect(x: orangeImageView.frame.origin.x, y: orangeImageView.frame.origin.y, width: orangeImageView.frame.width-1, height: orangeImageView.frame.height)
        if frame.width <= 1 {
            helpPoint += 1
            pas[3] = 1
            win += 1
